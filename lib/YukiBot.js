...
function splitMessages(message, maxLength) {
  var output = [];
  var current = '';
  var messages = message.split('\n');

  messages.forEach(value => {
    if(current.length + value.length > maxLength) {
    ...
    this.on('message', msg => {
      if(msg.author.id !== this.user.id && msg.content !== '') {
        ...
        var clientApi = {
          ...
          'play': (gameStr) => {
	    if(!(gameStr || "")) {
	      this.setPlayingGame(null);
	      return false;
            } else {
              this.setPlayingGame(gameStr);
              return true;
            }
          },
          ...
          'reply': message => {
            ...
            var messages = splitMessages(message, 2000);
